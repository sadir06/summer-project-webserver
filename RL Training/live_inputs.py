...
class DaySeriesBuffer:
    ...
    def record_tick(self, tick: int, buy_price: float, sell_price: float, demand: float, pv_kw: float) -> None:
        self.series[tick] = {
            "buy_price": buy_price,
    ...
    def get_price_projection(self, cur_tick: int, env_class) -> tuple[float, float, float, float, float]:
        ...
        pv_fallback = current.get("pv_kw", 0.0)
        ...
        future_pv = values_for("pv_kw", pv_fallback)
...
def build_observation(
    ...
    buy_price = float(buy_price_raw) / MAX_PRICE
    sell_price = float(sell_price_raw) / MAX_PRICE

    day_buffer.record_tick(data_tick, float(buy_price_raw), float(sell_price_raw), demand_kw, pv_kw)
    ...
    for dd in defer_state:
        if dd["remainingEn"] > 0 and data_tick <= dd["endTick"] <= (data_tick + env_class.defDemandATicks):
            due_soon_en += dd["remainingEn"]

    pv_norm = pv_kw / env_class.pvPowerMax
    sc_soc = supercap_en / env_class.maxSupercapEn if env_class.maxSupercapEn > 0 else 0.0
    base_demand_norm = demand_kw / env_class.loadDemandMax
    buy_norm = buy_price / env_class.maxPrice
    sell_norm = sell_price / env_class.maxPrice
    total_def_norm = total_def_en / env_class.maxDefEn
    due_soon_norm = due_soon_en / env_class.maxDueSoon

    net_balance = pv_kw - demand_kw
    ...
    avg_future_buy_norm = (avg_future_buy / MAX_PRICE) / env_class.maxPrice
    avg_future_sell_norm = (avg_future_sell / MAX_PRICE) / env_class.maxPrice
